...
class BrokerEvents(object):
    ...
    def __init__(self, options: BrokerEventsOptions) -> None:
        ...
        self.consumers = self.query_consumers_http(management_uri=options.management_uri)
    ...
    @staticmethod
    def query_consumers_http(management_uri: str) -> ConsumerList:
        url = urljoin(management_uri, "/api/bindings")
        reply = requests.get(url=url, timeout=5)
...
def load_json(logger: Logger,  # type: ignore[return]
              path: str = "/etc/is-broker-events/options.json") -> BrokerEventsOptions:
    try:
